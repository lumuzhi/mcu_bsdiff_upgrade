...
//    uint8_t *inBuf;
    SizeT inProcessed;
    SizeT outProcessed = size; // size = 128
    ELzmaFinishMode finishMode = LZMA_FINISH_ANY;
    ELzmaStatus status;
    SRes res;

    res = lzma_decompress_init(pf); // 里面调用vfread
    if (res != 0)
    {
    ...
    //获取当前文件读指针
    vfgetpos(pf, &position);    // position 偏移量，返回当前指向地址,
    ...
    // add for linwanrong 2024-12-17
    // 该128是每次还原的长度，如果外面修改这儿也要修改
    uint8_t buff[128] = {0};
    vfread(pf, buff, sizeof(buff));
    // bs_printf("inProcessed:%d", inProcessed);

    //解压数据
    if (inProcessed > 0 || inProcessed == 0)
    {
        res = LzmaDec_DecodeToBuf(lz_state, buffer, &outProcessed, buff, &inProcessed, finishMode, &status);
