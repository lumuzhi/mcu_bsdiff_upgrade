...
/* 以下由用户实现 */
#define bs_printf(...)  rt_kprintf(__VA_ARGS__)
#define bs_malloc(size) rt_malloc(size)
#define bs_free(ptr)    rt_free(ptr)
//#define bs_printf(...)
 ...
 */
extern int iap_patch(const uint8_t* old, uint32_t oldsize, const uint32_t patch,
                     uint32_t patchsize);
